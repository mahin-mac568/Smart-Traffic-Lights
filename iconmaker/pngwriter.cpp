...
void pngwriter::plot_text(char *face_path, int fontsize, int x_start, int y_start, double angle, char *text, double red, double green, double blue)
{
  ...
  {
    std::cerr << " PNGwriter::plot_text - ERROR **: FreeType: Font was opened, but type not supported." << std::endl;
    return;
  }
  else if (error)
  {
  ...
  for (n = 0; n < num_chars; n++)
  {
    ...
    if (error)
    {
      std::cerr << " PNGwriter::plot_text - ERROR **: FreeType: Set char size error." << std::endl;
      return;
    }
    ...
    if (error)
    {
      std::cerr << " PNGwriter::plot_text - ERROR **: FreeType: Render glyph error." << std::endl;
      return;
    }
...
int pngwriter::get_text_width(char *face_path, int fontsize, char *text)
{
  ...
  if (error)
  {
    std::cerr << " PNGwriter::get_text_width - ERROR **: FreeType: Could not init Library." << std::endl;
    return 0;
  }
  ...
  if (error == FT_Err_Unknown_File_Format)
  {
    std::cerr << " PNGwriter::get_text_width - ERROR **: FreeType: Font was opened, but type not supported." << std::endl;
    return 0;
  }
  else if (error)
  {
    std::cerr << " PNGwriter::get_text_width - ERROR **: FreeType: Could not find or load font file." << std::endl;
    return 0;
  }
  ...
  for (n = 0; n < num_chars; n++)
  {
    ...
    if (error)
    {
      std::cerr << " PNGwriter::get_text_width - ERROR **: FreeType: Set char size error." << std::endl;
      return 0;
    }
    ...
    if (error)
    {
      std::cerr << " PNGwriter::get_text_width - ERROR **: FreeType: Render glyph error." << std::endl;
      return 0;
    }
...
int pngwriter::bilinear_interpolation_read(double x, double y, int colour) const
{
  ...
  */
  ...
  if ((!attop) && (!atright))
  {
    ...
    return (int)(f1 * g1 * this->read(intx, inty, colour) + f * g1 * this->read(intx + 1, inty, colour) + f1 * g * this->read(intx, inty + 1, colour) + f * g * (this->read(intx + 1, inty + 1, colour)));
  }
  ...
  if ((atright) && (!attop))
  {
    ...
    return (int)(f1 * g1 * this->read(intx, inty, colour) + f * g1 * (2 * (this->read(intx, inty, colour)) - (this->read(intx - 1, inty, colour))) + f1 * g * this->read(intx, inty + 1, colour) + f * g * (2 * (this->read(intx, inty + 1, colour)) - (this->read(intx - 1, inty + 1, colour))));
  }
  ...
  if ((attop) && (!atright))
  {
    ...
    return (int)(f1 * g1 * this->read(intx, inty, colour) + f * g1 * this->read(intx + 1, inty, colour) + f1 * g * (2 * (this->read(intx, inty, colour)) - this->read(intx, inty - 1, colour)) + f * g * (2 * (this->read(intx + 1, inty, colour)) - this->read(intx + 1, inty - 1, colour)));
  }
  ...
  return (int)(f1 * g1 * this->read(intx, inty, colour) + f * g1 * (2 * (this->read(intx, inty, colour)) - (this->read(intx - 1, inty, colour))) + f1 * g * (2 * (this->read(intx, inty, colour)) - this->read(intx, inty - 1, colour)) + f * g * (2 * (2 * (this->read(intx, inty, colour)) - (this->read(intx - 1, inty, colour))) - (2 * (this->read(intx, inty - 1, colour)) - (this->read(intx - 1, inty - 1, colour)))));
...
void pngwriter::plot_text_blend(char *face_path, int fontsize, int x_start, int y_start, double angle, char *text, double opacity, double red, double green, double blue)
{
  ...
  if (error)
  {
    std::cerr << " PNGwriter::plot_text_blend - ERROR **: FreeType: Could not init Library." << std::endl;
    return;
  }
  ...
  if (error == FT_Err_Unknown_File_Format)
  {
    std::cerr << " PNGwriter::plot_text_blend - ERROR **: FreeType: Font was opened, but type not supported." << std::endl;
    return;
  }
  else if (error)
  {
  ...
  for (n = 0; n < num_chars; n++)
  {
    ...
    if (error)
    {
      std::cerr << " PNGwriter::plot_text_blend - ERROR **: FreeType: Set char size error." << std::endl;
      return;
    }
    ...
    if (error)
    {
      std::cerr << " PNGwriter::plot_text_blend - ERROR **: FreeType: Render glyph error." << std::endl;
      return;
    }
